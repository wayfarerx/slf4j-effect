 ...
 */
trait OldLoggerFactory {

  /** The exposed logger factory API. */
  val loggerFactory: OldLoggerFactoryApi[Any]
 ...
 */
object OldLoggerFactory extends (OldLoggerFactoryApi[Any] => OldLoggerFactory) with OldLoggerFactoryApi.Service[OldLoggerFactory] {
  ...
  trait Live extends OldLoggerFactory {
    ...
    final override val loggerFactory: OldLoggerFactoryApi[Any] = new OldLoggerFactoryApi.Service[Any] {

      /* Return a logger named according to the name parameter. */
      override def apply(name: String): Result[OldLogger] =
        Task(slf4jLoggerFactory.getLogger(name)) map (OldLogger.Live(_, self.blocking, self.console))
